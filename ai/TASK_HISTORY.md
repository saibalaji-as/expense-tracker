...
- Verified: app tsc + functions tsc clean. **Pending: redeploy `functions:updateCircle`; retest: remove participant-only member → balances re-tally; payer stays blocked with visible reason.**

## 2026-07-19 - Circle Splits Phase 3: linked-entry model REVERTED → settle auto-log + widget Circle button
- Product owner reverted the same-day linked-entry model ("logging then deducting after the trip is an unnecessary flow"). FINAL accounting: circle bills live only in the circle while active (no Daily entries, no account deductions, no checkboxes anywhere); on Settle Up each member's device AUTO-LOGS one Daily expense (`source: 'circle-settle'`, category Miscellaneous — no prompt) with the full Share Summary text + `[circleId]` in the comment (comment tag = cross-session dedupe; small same-user-two-devices race accepted). `CircleSyncService.#autoLogSettledCircle` replaces `#trueUpSettledCircle`; the post-share modal is gone (settle toast + settled-state info card explain the auto-log).
- Removed: `ExpenseEntry.circleId/circleExpenseId` + `'circle'` source, all mirror/reconcile/`linkEntryToCircle` machinery, `widgetCircleLinkPending$`, Daily checkbox + `canManageEntry` circle guard, circle-form category/account (mirror) block, widget expense-form checkbox, worker circleId-entry skip, 16 i18n keys. KEPT: member management + `updateCircle removeMemberId` guard, `computeMyShareOfExpense`/`computeShareOwedToOthers` utils + tests (pure, useful for future unequal splits), `spenza_active_circles_v1` widget cache, AppShell global circles listener (auto-log must fire without opening /splits).
- NEW widget flow (user-specified): dedicated **Circle row button** on the home-screen widget — 2-row layouts only (`widget_circle_row` TextView appended to `expense_widget.xml` + `_tall.xml`, GONE by default; quick/compact 1-row layout intentionally skipped, no space), label "◎ <trip> · Circle expense" (or "· N trips"), visibility bound to the circles cache. Opens `ExpenseWidgetActivity` in new `WIDGET_AMOUNT_KIND_CIRCLE` mode: circle picker/name + amount + description ONLY (category/account hidden); guard-finishes if the cache emptied. Queues `{kind:'circle-expense', circleExpense:{id,circleId,amount,date,description}}` via `enqueueCircleExpense` (no WorkManager schedule); `WidgetExpenseSyncWorker` skips the kind (Firestore-only payload); app flush consumes + emits `widgetCircleExpensePending$` (ReplaySubject) → `CircleSyncService.#pushWidgetCircleExpense` setDoc with the item id as doc id (idempotent; durable offline via persistentLocalCache). Split = equal among ALL members, paidBy = the widget user.
- Verified: app tsc clean, functions tsc clean, vitest 104/104 (settlement 14, expense-store 49, daily 41), layout XML parse-checked; Java NOT compiled (no SDK). Pending: prod build, Gradle build + device test (Circle row appears only with an active circle; settle → auto-log on every member device), functions redeploy unchanged from Phase 2 needs (updateCircle), `graphify update .`.

## 2026-07-19 - Circle Splits Phase 2: linked entries ("full now, true-up on settle"), member management, Daily checkbox, widget checkbox — **REVERTED same day, see Phase 3 above**
- ACCOUNTING DECISION (user-approved after explicit double-count warning): a circle bill paid by me creates a personal Daily entry for the FULL amount immediately (real cash out; optional account deduction). On Settle Up each such entry is automatically reduced to my per-head share (updateEntry's account-delta reversal credits back what friends owe). Shares of OTHERS-paid bills post separately via the (reworded) post-share modal using new `computeShareOwedToOthers` — invariant: own-paid trued-up share + owed-to-others == total share (unit-tested). Rejected alternatives: share-only-immediately (account/cash view wrong), full-with-no-true-up (budgets permanently overstated).
- Model: `ExpenseEntry` gains `circleId`/`circleExpenseId` + source values `'circle'`; util gains `computeMyShareOfExpense`/`computeShareOwedToOthers` (shared paise-exact `expenseSharesPaise`; 14 tests).
- CircleSyncService is now the linked-entry owner: mirror create on addExpense (category+account chosen in circle form when paidBy=me), snapshot-driven `#reconcileMirrors` for the OPEN circle (heals bills re-attributed to me, remote edits/tombstones, unpushed widget entries), `#trueUpSettledCircle` on circles-listener settled transition (session Set guard, idempotent, retries on failure), `linkEntryToCircle` public bridge, Preferences cache `spenza_active_circles_v1` ({email, circles[{circleId,name,myMemberId,memberIds}]}) for the native widget. AppShell starts the listener app-wide after session restore (Daily checkbox + true-up must not wait for /splits).
- Member management: circle-detail owner modal (add by name; remove disabled client-side when member has live bills); server `updateCircle` gained `removeMemberId` — transactional expense scan, 409 'Member has expenses', owner unremovable, claimed-uid removal also drops memberUids. Deliberate: new members join FUTURE bills only (edit a bill to include them retroactively); auto re-tally is inherent (computed signals).
- Daily: "Circle expense" checkbox (visible when active circles exist; name label when 1, select when many) → entry tagged `circleId` then `linkEntryToCircle` (offline-safe: healed on next circle open). `canManageEntry` now blocks `circleId` entries (managed from Splits, like debt-payments from Finances).
- Widget (native, NOT built — no SDK in sandbox): `WidgetExpenseUtils.activeCircles` (email-scoped cache read), `ExpenseWidgetActivity` checkbox "Circle expense · <name>" (ThemedDropdown picker when >1; hidden in credit/cc-payment modes) tagging `entry.circleId`; `WidgetExpenseSyncWorker` keeps circle-tagged entries queued (app-only resolution, cc-payment pattern); store flush emits `widgetCircleLinkPending$` (ReplaySubject 100) consumed by CircleSyncService.
- i18n: 17 new keys + 3 reworded settle keys × en/ta/hi. Verified: app tsc clean, functions tsc clean, vitest 104/104 (settlement 14, expense-store 49, daily 41).
- Pending: prod build; functions redeploy (updateCircle changed) + rules already deployed set unchanged; Gradle build + device test (widget checkbox → app flush → partner sees bill; settle true-up on both devices); `graphify update .`. Known gaps: mirrors for bills logged by OTHERS naming me payer appear only when I open the circle; family partner edits of my circle-linked entries don't mirror (partner UI blocks them anyway).

## 2026-07-19 - Circle Splits Phase 1: N-member group expense sharing + settlement (new feature)
- BA decision record (full spec: `docs/circle-splits-plan.md`): trip-settlement use case; multi-user synced groups chosen deliberately as a growth loop (Circle Link lands on the PWA — Google sign-in, no APK). Feature naming: **Splits** (screen), **Circle** (group), **Circle Link** (invite), **Settle Up**.
- Non-negotiable design rules: (1) FREE tier — no Pro gate anywhere (paywalled invite kills the viral loop; Family stays Pro); (2) placeholder members mandatory (settlement never blocks on app adoption; joiners claim seats); (3) budget integrity — circle spends NEVER enter `expenses[]`; only the user's per-head share posts as one ExpenseEntry (`source: 'circle-settle'`, comment carries `[circleId]` for dedupe) on Settle Up; (4) equal split only in Phase 1 with per-expense participant selection.
- Explicitly rejected: reusing the 2-role Family Ledger for N members; dumping group spends into personal budgets; naive pairwise IOUs instead of minimal transfers.
- Firestore: `circles/{id}` (members map + `memberUids` for rules/queries, Functions-only writes), `circles/{id}/expenses/{id}` (author-owned client writes, tombstones only, write-locked once settled), `circleInvites/{code}` (multi-use, 7-day TTL — differs from single-use 24h family invites on purpose). Rules helpers `isCircleMember/isActiveCircleMember/isCircleOwner`.
- Functions `functions/src/circles.ts`: createCircle / createCircleInvite (reuses live code so the shared link stays stable) / redeemCircleInvite (transactional seat claim, idempotent for rejoin) / previewCircleInvite (non-members can't read circle docs under rules — join screen needs name+seats) / updateCircle / settleCircle.
- Client: `core/models/circle.model.ts`; PURE `core/utils/circle-settlement.ts` — integer-paise math, deterministic remainder distribution (memberId sort) so all devices show identical numbers, greedy min-transfer (≤ n−1 transfers), share-summary text builder (11 unit tests); `CircleApiService`; `CircleSyncService` (my-circles `array-contains` listener + per-circle expense listener via shared `firestore-db.ts`; no ExpenseStore/Drive involvement).
- UI `features/splits/`: `/splits` list + create sheet + code entry; `/splits/:id` tabs Expenses/Balances/Settle Up (invite share, summary share, owner Settle Up, post-my-share modal with category picker); public `/join/:code` (seat claim or join-as-new; unauthenticated → code parked in Preferences `spenza_pending_circle_join_v1` → resumed by AppShell after sign-in). Desktop nav + mobile top-bar icon. 88 i18n keys × en/ta/hi.
- Verified: app tsc clean, functions tsc clean, circle-settlement 11/11, expense-store 49/49 (ExpenseEntry.source union gained `'circle-settle'`).
- Pending: prod build, deploy (rules + 6 circle functions), Firestore composite-index check on first `array-contains` query, two-device flow test. Phase 2 (widget checkbox + member-activity FCM nudge) not started.

## 2026-07-19 - CC statement snapshot + user confirmation (reminder showed live outstanding, not the bill)
- User report: due-date reminder showed total card spend to date instead of the amount billed on the statement date (bill day = 3rd). Analysis: the planner's `statementDueAmount` DID subtract post-bill entry charges, but (a) ignored `DebtAdjustment`s entirely (fees/cash-advances/refunds after the bill skewed the amount — `credit-card-insights.statementDueWithAdjustments` netted them, the reminder path didn't), and (b) the amount was always a live reconstruction — bank-side interest/fees/EMI conversions Spenza never sees made drift structural, with no way to correct it. This is the app-side half of the planned "Phase 2 statement capture" (docs/notifications-screen-and-cc-cycle-plan.md §7) without the SMS classifier.
- Fix: `DebtAccount.statement` snapshot (`CardStatement`: billDateStr, amount, minDue?, source 'derived'|'user', updatedAt; one per card, replaced each cycle — full cycle history deliberately deferred, user-approved). Pure util `core/utils/credit-card-statement.ts`: `deriveStatementAmount` reconstructs outstanding AS OF the bill date (post-bill charges/adjustments subtracted, post-bill payments ADDED BACK — the statement is fixed, payments only reduce what's left), `statementRemainingDue` (statement − payments since bill, clamped 0 — user-approved semantics), `pendingDerivedStatement`, `statementIsCurrent`. Date rule: activity ON the bill day belongs to the statement (`> billDateStr` is post-bill).
- Store: debounced web+native effect → `ensureCardStatements()` auto-snapshots (`source:'derived'`) when a card's bill day passes; idempotent per cycle so the effect settles; persists to Drive; family-safe (whole-record updatedAt-wins merge, both devices derive the same numbers). `confirmCardStatement()` validates (0 ≤ minDue ≤ amount) and stores `source:'user'`.
- Finances: amber banner in the cycle box for derived snapshots ("estimated" + Confirm / Edit inline form), green confirmed line after; `cardStatementDue` prefers snapshot-minus-payments over live derivation.
- Reminders: `buildCreditCardReminderPlan` gained an `adjustments` param (wired from the store effect through `scheduleCreditCardDueReminders`); amount priority = snapshot (labeled "(estimated — confirm in Spenza)" unless user-confirmed) → adjustment-netted estimate → raw outstanding (no bill day); statement minDue overrides the card's static minimum everywhere including the overdue nudge; bill-day notification copy now asks to confirm the statement.
- Rejected: deriving at reminder-fire time (native one-shots can't recompute), Firestore-side snapshots (data is Drive-owned), full ccCycles[] ledger now (scope; snapshot design allows adding it later without migration).
- Verified: tsc clean; credit-card-statement 14/14 (new), credit-card-reminders 20/20 (6 new), insights 13/13, expense-store 49/49; Java widget confirmed to pass debt JSON through whole (snapshot survives worker Drive merge). Pending on user machine: prod build, Android rebuild + device test, `graphify update .`.

## 2026-07-16 - Daily UX fixes: payment-source tags in entry views + overspend banner → persistent toast
- Payment source tag (user report: grouped-entry dialog rows had no account/card indication): new `paymentSourceLabel(entry)` in daily-expense — account name for account-paid entries, "«Card name» · Credit card" for card-paid (`daily.paymentCreditCard`). Applied to ALL THREE entry-display sites: main list group row, single-entry detail dialog chip (icon switches wallet-cards ↔ credit-card via `isCardEntry`), and each row of the grouped-entries detail dialog. Verified no other feature renders entry payment sources.
- Overspend warning UX (user report: banner at top of Daily page is easy to miss): the inline `lastMonthOverspend` banner block was removed from the template; a constructor effect now fires `feedback.warning(..., persistent=true)` — the shared toast (bottom overlay, z-70, role=alert) stays until the user taps ✕. Shown once per category per session (the effect marks the type acknowledged immediately, flipping the computed to null). `dismissOverspendWarning` deleted. New `daily.overspendToast.title/detail` i18n keys ({category}/{overspent}/{spent}/{limit}) × en/ta/hi — the old banner was hard-coded English.
- E2E TC-DAILY-05 still matches (selector accepts any `role="alert"` with over/budget/warn text; the warning toast has role=alert). daily-expense specs 41/41, ng build dev clean. **Pending: `graphify update .`.**

## 2026-07-16 - Credit-card insights: cycle view, utilization alerts, refund auto-match, cashback tracker
- Four features on top of the same-day DebtAdjustment work, all driven by pure helpers in new `core/utils/credit-card-insights.ts` (13 unit tests):
  - **Statement-cycle view** (Finances, per card with `billGenerationDay`): current cycle range (last bill → next bill via `lastOccurrenceOnOrBefore`/`nextOccurrence` reused from credit-card-reminders), net spend this cycle (purchases + fee/cash-advance adjustments − refunds/cashback), statement due (`statementDueWithAdjustments` — supersedes reminders' `statementDueAmount` by netting DebtAdjustments; the reminder planner itself is unchanged), and "Pay by <due date>".
  - **Utilization alerts at 30%/80% of creditLimit**: `crossedUtilizationThreshold` fires only on upward crossings (re-arms after paying down). Detection is an ExpenseStore effect over `debts()` — one implementation covers manual entries, widget flushes, adjustments, AND partner records applied from the family ledger; first effect run only snapshots so cold-start loads never alert; unseen card ids are snapshotted, not compared. Emits `cardUtilizationCrossed$` (budget-events.ts); `LocalNotificationService` subscribes (gated by budgetWarningsEnabled + permission, 6h dedupe per card+threshold, budget-alerts channel, routes to /finances; web falls back to the Notification API).
  - **Refund auto-match**: `matchRefundCandidates` — same card, exact 2-dp amount, ≤90 days before the refund date, purchases already claimed by another refund's `linkedExpenseId` excluded, closest-first max 3. Finances refund form shows tappable suggestion chips (amount/date mirrored to signals for OnPush); save passes `linkedExpenseId` only if the chosen candidate is still valid; history rows show "↩ <comment/type> · <date>".
  - **Cashback tracker**: new `DebtAdjustmentKind 'cashback'` (outstanding ↓ like refund, ledger-compatible payload — no sync changes needed); `cashbackEarned` total per card shown in the cycle block (or standalone when no bill day). Kind picker, per-kind hint, history sign handling via `isCardCredit`.
- 9 new i18n keys × en/ta/hi (`finances.cycle.*`, cardAdjust cashback/match keys).
- Verified: credit-card-insights 13/13, utils+store+features 286/286, ng build dev clean.
- **Pending (user machine): `graphify update .`; device test — cross 30% on a limit-set card and confirm the notification; check cycle numbers against a real statement.** Note: utilization alerts reuse the budgetWarningsEnabled preference rather than a new toggle (deliberate; add a dedicated pref only if users complain).

## 2026-07-16 - DebtAdjustment: card refunds, cash withdrawals, fees (no more refund-as-expense)
- Product gap (user request): a refund credited to a card had to be logged as an expense (inflating outstanding AND monthly spend), and cash withdrawn from a card had no representation. Fix extends the proven AccountBalanceAdjustment pattern to cards.
- Model: `DebtAdjustment` in debt-account.model.ts — kinds `refund` (outstanding ↓, clamped at 0; audit record keeps the full amount so post-bill-payment refunds are recordable), `cash-withdrawal` (outstanding ↑ + receiving asset account ↑ atomically via `applyAccountDeltas`; NOT an expense — spending the cash later logs normally, no double count), `charge` (fees/interest, outstanding ↑). Optional `linkedAccountId` (withdrawal target), `linkedExpenseId` (refund traceability), `reason`, full actor audit fields. None of the kinds create an ExpenseEntry — that is the core correction.
- Store: `debtAdjustments` state array + `recordDebtAdjustment()` (validates active credit card; withdrawal requires unarchived account). Wired through every persistence surface: BackupDocument (`debtAdjustments?` field + buildInitialDocument + isBackupDocument + buildBackupDocument), local cache load, `preserveCachedFinanceArrays` healing, `mergeBackupDocumentForFamily` (add-only + tombstone filter), clearLocalData/new-file resets, `flagPendingDelete` union += 'debt-adjustment', `deleteDebt` cascades its adjustments (mirrors deleteAccount).
- Family ledger: new `LedgerRecordType 'debt-adjustment'` — LedgerStateView/diff/apply/localSigFor/tombstone paths in family-ledger.util.ts; store's ledgerStateView + changes$ patch. No Firestore rules change needed (ledger rules are type-agnostic). family-state-merge.ts (legacy Drive merge) adds mergeAddOnly for the field.
- Finances UI: "Refund / Cash" button per card → inline form (kind ThemedSelect, amount, date, account picker only for cash-withdrawal via `cardAdjustKind` signal mirroring the form control for OnPush, reason otherwise, per-kind explainer hint) + "Refunds & adjustments" history list (−green refunds / +destructive charges, linked account name, localOnly chip). 18 i18n keys × en/ta/hi (`finances.cardAdjust.*`, `finances.feedback.cardAdjustment*`).
- Verified: family-ledger.util spec 24/24 (3 new debt-adjustment cases), expense-store spec 49/49 (5 new balance-math mirrors incl. net-worth invariant for withdrawals), features suites all green (217 total), ng build dev clean. Same pre-existing unrelated failures (local-notification vi.mock, fcm, ai-insight).
- Deliberately deferred: analytics category-netting of refunds against the original purchase's category (the original entry stays an expense; with edit/delete now available the user can correct truly wrong entries), widget/SMS refund detection (needs native Java + Gradle, unavailable in this env), edit/delete of adjustment records (append-only audit like account adjustments).
- **Pending (user machine): `graphify update .`; device test: refund → outstanding drops, withdrawal → card + account both move; if family CF/rules deploy from the ledger work is still pending, this rides along with it (no new deploy artifacts of its own).**

## 2026-07-16 - Card-linked expenses now editable/deletable with charge reversal
- Product rule change (user request): credit-card purchase entries (`debtId`, source ≠ 'debt-payment') can be edited and deleted from the Daily Expense page; only repayments (`source: 'debt-payment'`) stay Finances-managed. Previous blanket block existed because ExpenseStore had no reverse/apply for card outstanding.
- Store: new `addDebtDelta`/`debtDeltasForEntryUpdate`/`debtDeltasForDeletedEntry` + `applyDebtDeltas` (signed deltas; positive net delta = new charge requiring an active card, same rule as `applyDebtCharges`; negative = reversal, honoured even on archived cards, outstanding clamped at 0 per the widget CC-payment clamp precedent). Wired into `deleteEntry`/`updateEntry`; the `debtId` throw condition removed, `debt-payment` throw kept.
- Component (daily-expense): `canManageEntry` only blocks `debt-payment`; `editEntry` pre-selects the card via `debt:<id>` selector value; `paymentAccountOptions` lists cards in edit mode too (plus the editing entry's own card if archived so saving doesn't force a relink); `selectedPaymentLink` accepts the editing entry's archived card; component `updateEntry` now resolves account XOR card explicitly (`accountId`/`debtId` both set from `selectedPaymentLink()` — stale `debtId` no longer survives a source change via the originalEntry spread).
- Family ledger untouched by design: debts/entries flow through `pushFamilyLedger` diff-reconciliation automatically.
- Verified: expense-store spec 44/44 (old "debtId blocked" mirror tests replaced with reversal-math tests incl. card→account, card→card, clamp), daily-expense specs 41/41, ng build dev clean. Pre-existing unrelated failures remain (subscription.service localStorage lint spec, fcm/local-notification e2e, ai-insight). **Pending: `graphify update .` (CLI unavailable in session env); device sanity check of edit/delete on a card entry.**
- Rejected alternative: allowing negative card outstanding on reversal — model forbids negative `remainingBalance` elsewhere; clamp chosen. Revisit when card refunds/credit-balance support lands (see BA analysis: planned `DebtAdjustment` refund/cash-withdrawal feature).

## 2026-07-16 - Widget daily budget now mirrors the app's formula
- Mismatch: app's Daily Expense hero uses `Math.round(monthlyIncome / 30)` (daily-expense.component.ts), but the widget derived `ceil(sum(limit.userPercentage% x income) / actualDaysInMonth)` — different figure whenever limits exist or the month isn't 30 days.
- Fix: `ExpenseWidgetProvider.DailyInsight.dailyBudget()` reduced to `Math.round(monthlyIncome / 30.0)`; limits array + daysInMonth helper removed. Comment marks the app component as the source of truth — change both together.

## 2026-07-16 - Widget tiers v2: orientation-aware sizing after on-device screenshots
- Evidence from real device: at 1 launcher row the 2-row layout rendered clipped (budget+progress visible, chips cut) because the quick/compact tier keyed off `OPTION_APPWIDGET_MIN_HEIGHT` (landscape height) with a 70dp cutoff, while the user's launcher reports ~90-110dp for one portrait row; at 2 rows (~155dp) the 110dp-budgeted layout left large empty bands with small 38dp chips.
- Fix pattern (reusable): never assume the 70n-30 cell formula. Build `new RemoteViews(landscapeViews, portraitViews)` and derive each orientation's tier from its exact reported height (portrait=MAX_HEIGHT, landscape=MIN_HEIGHT): <115dp quick/compact, 115-149 standard (trimmed to fit ~115), >=150 roomy `*_tall.xml` variants (bigger chips/type). maxResizeHeight=200dp: blocks 3-row on dense grids (>=180dp) while allowing tall-cell 2-row resizes.
- Layout hygiene that made this cheap: standard and tall variants share every view id, so providers bind once per tier with zero conditional binds; tall files are script-generated from the standard files with size-only substitutions.
- Also: glass surface alpha lowered to restore wallpaper bleed-through per reference image.

## 2026-07-16 - Widget two-way sync: partner expenses on the widget via FCM overlay
- Feature request: partner-logged expenses should appear on the other member's home-screen widget without opening the app ("two-way communication between app and widget").
- Design principle: DISPLAY-ONLY overlay, explicitly NOT a new sync path (AI_RULES: mutations flow through ExpenseStore only). Losing an FCM message loses nothing but widget freshness.
- Implementation: (1) CF `notifyPartnerLedgerWrite` (v2 Firestore trigger on `families/{id}/ledger/{recordId}`, new functions/src/ledger-push.ts) — forwards expense records (live: date ≤ 7 days old to cap bulk-reconcile bursts; tombstones: always) as high-priority FCM DATA messages to the other member's native tokens (users registry: ownerUid + platform=='native'; writer uid excluded). (2) `MyFirebaseMessagingService` (existed; data payloads were previously ignored) → validates family mode + not-own-email → `PartnerPendingStore.upsert` (`spenza_widget_partner_pending_v1`, cap 100/14d, email-scoped, wiped on account change) → `ExpenseWidgetProvider.updateAll`. (3) Widget render = snapshot ⊕ overlay ⊕ own queue: `PartnerPendingStore.overlayExpenses` overrides/removes snapshot copies by id (partner edits/deletes reflected) and prunes records received at/before snapshot `savedAt` (supersede rule — Java isoNow and Angular toISOString both emit yyyy-MM-dd'T'HH:mm:ss.SSS'Z', parse verified; parse failure fails open, id-dedup still prevents double counts).
- Constraint: requires partner's device to have notifications enabled (FCM token registered); silently degrades to app-open behavior otherwise. `onNewToken` TODO left as-is (app re-registers on start).
- Verification: functions build clean; Java not compiled in env — local Gradle build required. **Pending: `firebase deploy --only functions` (first Firestore-trigger deploy may prompt to enable Eventarc/Cloud Run APIs), Gradle build both devices, test: partner logs/edits/deletes with your app closed → widget updates in seconds; open app → identical totals (no double count).**

## 2026-07-16 - Widget-flush update-revert fix: persisted verify flags for familySynced items
- Field failure: widget-logged expense (owner, app closed) → partner received and EDITED it → owner opened app → both devices reverted to the original. Cause: the flush inserted the capture-time queue payload as fresh local truth; the owner's persisted ledger copy had never seen the record (CF wrote it server-side while the app was closed), so the diff pushed the stale v1 over the partner's v2, and partner LWW-applied the revert.
- Fix (user's flag method, third application): persisted verify flags `spenza_family_pending_verify_v1` — flush flags every `familySynced`-tagged expense/adjustment doc id; `diffLedgerState` never pushes a flagged record (`pendingVerify` option); `pushFamilyLedger` first runs `FamilySyncService.verifyDocs()` (SERVER-only `getDocsFromServer` in `documentId() in` chunks of 10 — a cache miss must not read as "CF never wrote it"): found docs are ingested + emitted via changes$ (newer partner version replaces the stale payload locally), missing docs clear the flag so the normal push delivers them. Offline: verify throws, flags persist, `familyPushPending` kept (verifyDeferred) so resume/online retries.
- Verification: app tsc clean; ledger util spec 23/23 (verify-skip, cleared-flag-push, stale-payload-corrected); expense-store 37/37. **Pending: prod build + `npx cap sync android` + Gradle both devices (Angular-only change — no CF/rules deploy needed); manual case: widget log (app closed) → partner edits → owner opens app → edited version must survive on both.**

## 2026-07-15 - Family Ledger Phase 2 cleanup (user-approved: cleanup only, derived balances deferred)
- Context: after the delivery hardening, user reports family sync working; one-day observation running. Scope deliberately excludes derived balances to keep the observation signal clean.
- Changes: (1) `WidgetExpenseSyncWorker` early-returns after the CF push in Firestore-family mode — the Java Drive-merge leg no longer runs there (retry until `familySynced` tag, queue consumed by app flush; also fixes widget-insight double count of untagged items present in both snapshot and queue). Single/legacy modes keep the Drive leg — it is their only background backup (uninstall-loss risk otherwise; see 2026-07-11 rejection note). (2) Legacy CF `syncWidgetExpenseToFamily` deleted from source + index exports — **manual step: `firebase functions:delete syncWidgetExpenseToFamily`** (both devices confirmed on the ledger build). (3) `families/{id}/state/{docId}` rules write-locked (read stays for one-time migration). (4) Flush simplified: `widgetFamilyPushNeeded`/`owesFamilyPush` deleted; flush always ends with `pushFamilyLedger()` (no-op when in sync). (5) Plan change: `family-state-merge.ts` KEPT — its utils power the family-mode Drive-load merge.
- Verification: app tsc clean, functions build clean, 69/69 tests (ledger util 20, expense-store 37, merge 12). Java not compiled in env — local Gradle build required. **Pending deploy: `firebase deploy --only firestore:rules,functions` (index changed), `firebase functions:delete syncWidgetExpenseToFamily`, `npx cap sync android` + Gradle build both devices. Re-run widget→closed-partner test after install (worker path changed). Derived balances = next pass, only after the observation window closes clean.**

## 2026-07-15 - Family Ledger delivery hardening: persisted delete flags, ack-only copy, no listener gate
- Field failures after Phase 1: (1) partner's app-logged expense never arrived until re-logged; (2) deleted expense resurrected after reopen, then deleted on both devices minutes later. User proposed the fix concept himself: a durable per-record delete flag checked on every operation — implemented exactly that, generalized.
- Root causes (Phase 1 delivery discipline, not the ledger model): pushes gated on the listener's first snapshot (`isPrimed`) so an early-session log pushed nothing; fire-and-forget commits (toast ≠ delivered); in-memory ledger copy + deleted-ids set collapsed the divergence guard to "ledger wins" after every restart (resurrection); Drive loads wholesale-replaced state and the racy absence-tombstone heuristic sometimes corrected it; settings.component's direct `getFirestore()` could downgrade the session to memory cache.
- Fixes: (1) **Persisted delete flags** `spenza_family_pending_deletes_v1` — flagged to disk BEFORE the mutation on deleteEntry/deleteDebtPayment(+linked expense)/deleteDebt/deleteAccount(+adjustments); apply never resurrects a flagged record, diff never re-uploads one and emits its tombstone; cleared on server-acked tombstone; acked tombstones win forever. (2) **Persisted ack-only ledger copy** `spenza_family_ledger_copy_v1` (sigs via `opSig` length+double-FNV hash) — snapshot docs with `hasPendingWrites` skipped, so diffs re-produce ops until acked and the guard survives restarts. (3) `primeNow()` prime-on-demand replaces the isPrimed gate. (4) `familyPushPending` true until server ack; `pendingAckCount` signal. (5) Family-mode Drive loads merge (mergeBackupDocumentForFamily: merge utils + tombstone/flag filter, enriched doc written back). (6) Absence-tombstone heuristic deleted. (7) settings.component → getSharedFirestore.
- Rejected: keeping absence tombstones alongside flags (redundant + the racy component); awaiting commit acks on the UI path (hangs offline).
- Verification: app tsc clean; family-ledger.util spec 20/20 including an end-to-end delete→restart→snapshot resurrection test; expense-store 37/37; family-state-merge 12/12 (utils now also used by the family Drive-merge). **Pending: same deploy/build/two-device matrix as Phase 1, plus: log → force-stop within 3s → reopen; delete offline → kill → reopen online. `graphify update .` (CLI unavailable in env).**

## 2026-07-14 - Family Ledger: centralized record-level sync (Phase 1) replaces full-state doc
- User mandate: widget/notification logs still not reaching partner (opening the app didn't sync; editing any expense did). Root-cause analysis: four bespoke sync implementations (app pushState transaction, widget CF full-state merge, app flush, snapshot merge) across three codebases; correctness was event-triggered. Also `families/{id}/state/current` holds the whole backup in ONE doc → 1 MiB ceiling. Full design + phases in `docs/family-sync-centralization-plan.md`.
- Architecture: `families/{familyId}/ledger/{type:id}` — one Firestore doc per record (expense/adjustment/debt-payment/account/debt + `limits`/`meta` singletons), idempotent upserts, tombstone deletes (`deleted: true`, doc deletes forbidden by rules), LWW per record. Firestore initialized with `persistentLocalCache` via new shared `core/services/firestore-db.ts` (`getSharedFirestore()` — subscription/reminder/family-sync services all switched; a stray `getFirestore()` first would silently downgrade to memory cache).
- Send path (ONE): `ExpenseStore.pushFamilyLedger()` — diffs local state vs the device's ledger copy (`diffLedgerState` in new pure `core/utils/family-ledger.util.ts`) and commits only missing ops (`FamilySyncService.commitRecords`, batched ≤400, fire-and-forget: with the persistent cache the SDK owns offline queuing/retry across restarts). Reconciliation-based: ANY call heals ANY gap. A debounced reconcile push runs after every ledger snapshot, so sync no longer depends on a code path remembering to push. Absence tombstones (record in ledger, missing locally) propagate deletions like removed debt payments, gated by `localDataHydrated` + capped (default 25, ALL dropped above cap) so a stale/partial hydration can never bulk-delete family history. Receipts stripped before push (device-private).
- Receive path (ONE): `FamilySyncService` (rewritten) listens to the ledger collection, maintains the ledger-copy map, emits changes with `prevJson`; `applyLedgerChanges` applies with a local-divergence guard — local un-pushed edit ≠ prevJson → incoming skipped, next diff push wins (LWW). Own echoes no-op. Local receipt preserved when incoming entry lacks one.
- Migration: first member attaching to an empty ledger (server snapshot) fans legacy `state/current` out into records (incl. deletedEntryIds → tombstones). State doc kept read-only as fallback; even without migration each device's diff push re-uploads its local state (self-healing across app-version skew).
- Native path: new CF `commitFamilyLedger` (functions/src/family-ledger.ts) accepts the SAME worker payload `{familyId, expenses[], adjustments[]}`, upserts record docs in a transaction (idempotent by existing-doc check), applies account/debt deltas to ledger records when present. `WidgetExpenseConstants.FAMILY_WIDGET_SYNC_URL` now points at it; legacy `syncWidgetExpenseToFamily` stays deployed for old APKs (old APK → state doc → old app reads it; no skew loss). familySynced tag semantics unchanged.
- Rejected: repointing the old CF at the ledger (old app versions only read the state doc → partner loss during skew); FCM wake of partner (Phase 3 idea); derived balances (Phase 2 — accounts/debts still sync LWW like before).
- Verification: app tsc clean, functions build clean, new family-ledger.util spec 15/15, expense-store 37/37, family-state-merge 12/12. **Pending (user machine): `firebase deploy --only functions:commitFamilyLedger,firestore:rules`, `npm run build` prod, `npx cap sync android` + local Gradle build (widget URL change is native), two-device matrix test (app↔app, widget→closed partner, notification→closed partner, offline log → reconnect, same-entry concurrent edit), `graphify update .` (CLI unavailable in session env). Phase 2 cleanup (delete Java Drive leg, family-state-merge.ts, state-doc code) only after the matrix passes.**

## 2026-07-11 - Widget→family closed-app sync: per-item familySynced tag, CF bootstrap, id-based queue consumption
- User scenario: expense logged from widget/notification with app closed frequently never reached the partner, or duplicated. User proposed the shipped architecture independently plus one addition — a per-item sync tag — which became the fix mechanism.
- Root causes (in order of impact): (a) `WidgetExpenseSyncWorker` ignored the `pushFamilyWidgetExpenses` boolean; when the Drive leg then succeeded it cleared the whole queue, permanently losing the partner push. (b) `syncWidgetExpenseToFamily` threw 409 when `families/{id}/state/current` didn't exist (nothing pushed until owner opened the app; before the 2026-07-11 clobber fix the boot push then wiped what the CF had written). (c) Queue clearing used wholesale `replaceQueue` from a stale snapshot — a widget save racing the worker's network round-trip was clobbered (loss) and app-side flush could double-consume (duplicates). (d) App flush skipped `pushFamilyState()` when consumed widget items were all local duplicates.
- Fixes: wrapper-level `familySynced=true` tag persisted via new `WidgetExpenseQueue.markFamilySynced(ids)` (read-merge-write by id); `requiresFamilyPush()` gate — untagged family items are Drive-merged for durability but never consumed and force `Result.retry()`; new `WidgetExpenseQueue.removeConsumed(activeEmail, ids)` replaces `replaceQueue` in the worker (also drops the active user's malformed id-less items, except cc-payments); Drive write skipped when nothing new merged (family-retry runs no longer rewrite the backup). CF bootstraps the state doc (revision 1, `accounts: []`, no delta application — merge-on-write boot pushes converge balances) and the NO_STATE_DOC/409 path was removed; transaction counters reset on retry. Angular flush: `owesFamilyPush(raw)` (wrapper tag ≠ true) triggers `pushFamilyState()` even on the duplicates-only path; widget-flushed ids added to `pendingSyncIds` → amber "Syncing" chip in Daily list (reuses the Finances `localOnly` badge pattern; CloudOff icon registered; `daily.syncPending` en/ta/hi).
- Rejected: holding family-pending items out of the Drive merge entirely (simpler ownership, but leaves the entry existing only in widget prefs if pushes fail for days — uninstall would lose it); near-duplicate (±amount/same-day) matching inside the family merge (risk of swallowing genuinely distinct expenses; inbox auto-match already covers the notification path).
- Backward compatibility: tag is additive on the queue wrapper — old queue items without it are treated as unpushed (worst case one redundant idempotent CF push); Angular normalizer keeps `raw` so the tag survives requeue and is stripped on consume (never enters the backup schema).
- Verification: app tsc clean, functions tsc clean, expense-store spec 37/37, family-state-merge 12/12, storage-key-contract 9/9. Pending: local Gradle build, CF redeploy + `firebase functions:list` verification, two-device closed-app test (log from widget with app killed → partner sees entry + no duplicates; check logcat tag `WidgetExpenseSync` for "Family push still pending" retries).

## 2026-07-11 - Widget redesign: 2-row cap, per-style theming, instant style sync
- User feedback loop: (1) widgets looked odd at 2 cells (old layouts designed for 3 cells; streak compact threshold 160dp meant 2-cell showed the bare strip); (2) after first redesign, expanding to 4 rows stretched tiny chips (match_parent chip columns + weight row → predicted-highlight painted a full-height bar); (3) final direction with reference image: restrict to 4x1/4x2, hide budget line at 1 row, and make glass/clay/neu/brutal match the app styles exactly.
- Decisions: dropped the interim "large tier" (maxResizeHeight caps growth instead); chips wrap_content and center in the weighted row; per-style accuracy done in two layers — `WidgetSurface` panel painters retuned to the reference, plus runtime drawable swapping for inner elements (chips/pills/flame badge/day dots) keyed off `WidgetTheme.style()`; brutal gets ink-bordered variants incl. checkbox day squares; glass gets translucent-white chips/pills; clay pastel ovals; neu same-color chips.
- Rejected: per-style bitmap-rendered chips (RemoteViews can't set background bitmaps without restructuring every chip into a FrameLayout); changing ProgressBar drawable per style (setProgressDrawable is not remotable).
- Sync gap found+fixed: ThemeService persisted pf-style/pf-palette but nothing refreshed widgets until the next expense — setStyle/setPalette now fire ExpenseWidget.refresh() (native updateAll cascades to the streak provider) after the Preferences write completes.
- Verification: app tsc clean; all res XML validated; script-checked every R.id/R.drawable/R.layout reference in providers against res/. Native Gradle build pending (no SDK in session env).

## 2026-07-11 - Family sync fix: state-doc clobbering made partner expenses invisible
- Bug report: partner logged an expense; it never appeared on the other (owner, Android) device; partner editing the entry didn't help.
- Root cause chain: (a) `FamilySyncService.pushState` blind-overwrote `families/{id}/state/current` with the pusher's full local doc; (b) owner's cached-boot path (`bootstrapDriveInBackground`) called `pushFamilyStateNow()` BEFORE `tryStartFamilySync()` (listener attach was in `finally`); (c) the listener skipped any snapshot whose `lastWriter.uid` was self. Net effect: any partner change pushed while the owner's app was closed got wiped from the state doc on the owner's next boot, and the replacement snapshot was skipped as an own-write — permanently invisible to the owner (partner kept their local/Drive copy, so no data loss, just divergence).
- Secondary defects fixed in the same pass: `pushFamilyState` silently no-op'd when `authService.firebaseUid()` was null (common on native cold start before silent Firebase re-sign-in), and a failed push was only console.error'd with no retry.
- Decision: merge-on-write, not ordering discipline. `pushState` now reads the stored doc inside the transaction and merges (new pure `core/utils/family-state-merge.ts`: entries union by id / newer-timestamp-wins / receipt-preserving, tombstone union capped at 1000, accounts+debts by `updatedAt`, adjustments+payments add-only). With that, push/listen ordering can no longer destroy data. The listener additionally always delivers the FIRST snapshot after attach (own-writer or not — merge is idempotent), so a boot push can't hide partner changes carried in the merged doc. Boot still attaches the listener before pushing to shrink the stale window. Failed/deferred pushes set `familyPushPending`, retried from `flushPendingChanges()` (already invoked on app resume + online events).
- Rejected alternative: reverting to per-entry activity deltas (pre-2026-06 design) — full-state doc with merge-on-write is simpler and self-healing (every successful push carries everything).
- Known residual: session-scoped `localDeletedEntryIds` means a deletion whose push fails before an app restart can be resurrected by a later merge (pre-existing; state-doc tombstones cover the normal path). Widget-side family push audited: it goes through the `syncWidgetExpenseToFamily` Cloud Function (`functions/src/widget-sync.ts`), which already does a transactional append into the state doc — NOT a blind overwrite — so widget-logged entries were also victims of the client-side boot clobber, now fixed.
- Verified: `family-state-merge.spec.ts` 12/12, `expense-store.service.spec.ts` 37/37, `npx tsc --noEmit -p tsconfig.app.json` clean. Two-device family test pending.

## 2026-07-05 - Notification Inbox Phase 1 (persist-first spend detections + /notifications screen)
- Problem: `SpendNotificationListenerService` only stored a dedupe fingerprint; swiping the review prompt (or "Clear all", or denied POST_NOTIFICATIONS) lost the detected transaction permanently. Users clearing notifications missed expense logs.
- Decision: persist-first inbox. Every classified detection is appended to device-local Preferences `spenza_notification_inbox_v1` BEFORE the prompt is posted; the prompt is only a pointer (carries the inbox id via new `WIDGET_INBOX_ID_EXTRA`). Full plan + Phase 2/3 backlog in `docs/notifications-screen-and-cc-cycle-plan.md`.
- PRIVACY decision: inbox items contain SMS-derived text, so they live ONLY in Capacitor Preferences — deliberately NOT in `spenza-backup.json` or family Firestore. Only the resulting ExpenseEntry/DebtPayment/adjustment syncs. Do not "fix" the inbox's non-syncing behavior.
- Java: new `NotificationInbox.java` (synchronized append/markStatus, 200-item / 60-day eviction, email-tagged items); listener appends with kind expense/income/salary/cc-spend/cc-payment; `ExpenseWidgetActivity` marks items `logged` + links the created entry/payment/adjustment id in all three save paths.
- Angular: `notification-inbox.model.ts` + pure `notification-inbox.util.ts` (15 vitest specs) + `NotificationInboxService` (write queue serializes read-modify-write; writes preserve other-account raw items; in-memory signal updates even when the raw record was evicted).
- Duplicate auto-match decision: pending expense-like item auto-marks `auto-handled` when an expense with same local date and amount within ±1 exists and isn't linked to another item; each entry matches at most one item. Runs from an effect on the Notifications screen — prevents nagging about manually logged expenses.
- `/notifications` screen: recovery banner (pending spend total in selected currency), day-grouped pending list, Log modal reusing store transaction boundaries (`addEntry` with acct:/debt: payment link and `source: 'notification-prompt'`; `adjustAccountBalance` increase with Salary tagging for the salary reminder; `recordDebtPayment` capped at card outstanding — same rule as widget auto-tally), dismiss, handled history, filters, and the listener enable/OS-settings setup card. App shell gains an Inbox top-bar icon with pending badge + desktop nav item.
- Verified: `tsc -p tsconfig.app.json` clean; new util spec 15/15; production build result recorded below.
- Rejected alternatives: storing the inbox inside the Drive backup (breaks the "notification text never leaves the device" promise); navigating to /daily with prefill for logging (modal keeps recovery one-tap and also covers income/cc-payment kinds).
- Pending: local Gradle build + device test (listener → inbox → badge → log/dismiss), plus the still-outstanding auth-fix device test from the entry below.

## 2026-07-05 - Fix first-install multiple sign-in popups on Android APK
- Symptom (user report): fresh APK install → tap Sign in on callback/landing screen → multiple Google popups, first attempt fails, second attempt succeeds. Web unaffected.
- Root cause chain (offline refresh-token flow, `ENABLE_NATIVE_OFFLINE_REFRESH=true`): first-time offline sign-in inherently shows two Google surfaces (Credential Manager picker + Drive/offline authorization consent). When the flow then throws — user dismisses the second sheet, or the backend `exchangeGoogleAuthCode` fails at runtime (preflight only proves deployment+secret PRESENCE, not secret validity) — the catch in `#nativeSignIn` chained ANOTHER interactive online picker in the same gesture. A picker relaunched right after a dismissed one auto-cancels on Android → "cancelled by user" → first attempt fails; second attempt works because credential+consent are now cached.
- Fixes in `auth.service.ts` (client-only, no backend change):
  1. `#isUserCancellation()`: user cancellation now ABORTS sign-in (rethrow) instead of chaining a second picker — user just taps Sign in again.
  2. `#offlineFlowFailedThisSession`: a post-picker offline failure (i.e. exchange failure) blacklists the offline flow for the session, so retries go straight to the proven single-popup online flow.
  3. `#ensureSocialLoginMode()` serializes re-initialization against any in-flight `initialize()` (constructor eagerly inits ONLINE; a concurrent offline re-init could race and leave the plugin in online mode → "Expected offline Google login response" after the picker).
- Verified: `tsc -p tsconfig.app.json` clean; `auth.service.spec.ts` 11/11.
- Rejected approach: removing the interactive online fallback entirely — kept for genuine non-cancel failures so a broken backend never bricks sign-in.
- Follow-up for device validation: `adb logcat | grep -E "AuthService|SocialLogin"` on first sign-in; if "exchangeGoogleAuthCode HTTP 400 … googleError" appears, the OAuth client secret value is wrong (preflight cannot detect this) — re-set `GOOGLE_OAUTH_CLIENT_SECRET` with the Web client (663004583066-vu5…) secret and redeploy the two functions.
...
## 2026-07-16 (perf fix) — Native cold-start spinner stuck for seconds (initial load)
- Symptom: Android app shows the "Loading your data…" spinner far too long on every first open after ~1h (access-token expiry window), even though a full local backup snapshot exists.
- Root cause: `App.bootstrapData()` awaited `authService.getTokenSilent()` on native BEFORE `tryLoadCachedStartupData()`. With the token expired, that call chains: Firebase JS chunk import → `authStateReady()` → Firebase ID-token network refresh → Cloud Functions `getGoogleAccessToken` round trip (cold start: seconds) → possible Credential Manager silent fallback (+ tokeninfo scope check). None of that is needed to render locally cached data. Worst case, the CF fetch had NO timeout: a hanging connection held the spinner until the 30s timeout showed "Couldn't load data" despite valid local data.
- Fix (app.ts): start `getTokenSilent()` WITHOUT awaiting; try local cache immediately; on cache hit render at once and pass the in-flight renewal promise to `bootstrapDriveInBackground()`, which awaits it before the first Drive call (preserves the "silent refresh settles before any Drive request can trigger interactive fallback" guarantee — background writes already use the silent deduped path via `#requestNativeSignIn`, so no picker risk). No-cache path (first run/new device) still awaits the token as before. Added `[Perf]` console.info logging ms-to-cached-render for on-device verification.
- Fix (auth.service.ts): `#mintGoogleTokenFromServer()` fetch now has `AbortSignal.timeout(12_000)` so a hanging network/CF can never hold callers indefinitely; abort → null → existing fallback path.
- Rejected: hiding the spinner while bootstrap still runs (previously attempted "show app while loading in background" — added complexity/races without reducing real work). This change removes the network work from the critical path instead.
- Verification: `tsc --noEmit -p tsconfig.app.json` clean. Pending: `rebuild-android.sh` + device test (check logcat `[Perf] Startup:` line; expect cached render well under 1s), `graphify update .` (CLI not available in this sandbox).

## 2026-07-17 (regression fix) — Cold-start perf change broke silent sign-in → sync failures, widget dupes, landing flash
- Field report after the 2026-07-16 perf build: (1) CC entry delete + refund adjustment reverted overnight, (2) no automatic Google sign-in popup + Drive sync dead, (3) widget entry duplicated with two "Syncing" rows, (4) widget stale until first log, (5) landing page on every app open.
- ROOT CAUSE (introduced by the perf change): `#requestNativeSignIn` deduped ALL concurrent sign-ins onto one promise regardless of mode. With the startup silent renewal now running in PARALLEL with bootstrap, any interactive `ensureToken()` fallback landing during it was swallowed by the silent attempt → resolved with no token and no UI → "Native sign-in did not return a token" → no popup ever, Drive sync silently dead (issue 2). Cascade: dirty local snapshot (yesterday's CC delete + refund) could not flush; a later `loadFromDrive` with a stale Drive doc reverted state (issue 1); with sync dead the widget snapshot stayed stale (issue 4).
- Fix (auth.service.ts): mode-aware dedupe — silent joins anything; interactive joins interactive; interactive during SILENT chains after it (reuse token if silent succeeded, else run a real interactive sign-in; still never two pickers). Also feature-detected `AbortSignal.timeout` (old WebView would throw → silent mint permanently dead).
- Issue 3 root cause (latent, exposed by faster startup): `flushPendingWidgetExpenses` had NO in-flight guard; loadFromLocalCache's fire-and-forget flush now overlaps the focus/resume-handler flush (~200ms) — both snapshot `entries()` before either patches → same queue item inserted twice. Fix (expense-store.service.ts): serialize via in-flight promise; concurrent callers join. Plus `dedupeExpensesById` heal in `applyBackupDocument` (written back to Drive) and `loadFromLocalCache` so existing duplicates self-clean — do NOT hand-delete dupes (id-filter delete removes both copies while debt reversal applies once).
- Issue 5 root cause (pre-existing, more visible with instant render): native cold start lands on '/' (no hash) → `isPublicPage()` true → landing renders shell-less on every open. Fix (app.ts): on native, '/' is public only after auth restore settles AND user is signed out; `redirectAfterDataAvailable` now routes '/' → /daily (or /limits income onboarding). Web landing behavior unchanged.
- Verification: tsc clean, expense-store 49/49, auth 11/11. Pending: rebuild + device test (silent renewal popup path, widget dupe heal, no landing flash). Recovery note: user must re-apply yesterday's lost CC delete + refund adjustment manually — the stale Drive doc overwrote them.

## 2026-07-17 (follow-up) — On-device "Sync & sign-in log" (no adb needed)
- User asked for an in-app way to see sync/auth errors instead of adb logcat. SyncDiagnosticsService already persisted the last 50 Drive `driveFetch` failures but had NO UI and no auth coverage.
- Added: (1) `setAuthDiagnosticSink` in auth.service (own sink to avoid module cycle; structurally same event shape) with failure records at the blind spots — silent refresh produced-no-token / threw, mint: no Firebase ID token, HTTP non-OK (with status), 200-without-token, fetch threw. (2) SyncDiagnosticsService registers both sinks BEFORE the async history load (startup renewal runs in parallel — registering after could drop the exact failure being diagnosed); #load now merges behind live-recorded events. (3) Settings → new "Sync & sign-in log" card (below account section, hardcoded EN like other debug UI): Show/Hide with count, per-event operation/time/status/attempt/retry/message, Copy all (clipboard), Clear.
- Toast deliberately NOT used for these: transient, and background sync failures would spam; the persisted log survives restarts and is shareable.
- tsc clean; auth 11/11, expense-store 49/49. Pending: rebuild + device look at Settings after a failed-sync repro.

## 2026-07-17 (diagnosis) — Sync log confirmed root cause: no Google refresh token on server
- User's on-device log showed: `findBackupFile 401` (stale persisted token rejected) then repeated `auth.mintToken 404`. Server code confirms 404 = no refresh token stored for the Google sub (or no Google identity on the Firebase token). So silent renewal can NEVER succeed for this account; every renewal falls back to Credential Manager and every ~1h boundary risks 401s.
- Why no refresh token: it is only stored by a successful OFFLINE interactive sign-in, and Google only returns refresh_token on FIRST consent. User's sign-ins have been completing via online fallback (preflight failure / offlineFlowFailedThisSession / prior consent).
- Added log coverage: `auth.offlinePreflight` (preflight non-400/threw → offline flow disabled), `auth.offlineExchange` (HTTP failure w/ body, 200-no-token, threw, sign-in-OK-but-no-refresh-token w/ re-consent instruction, and a success breadcrumb), mint failures now include response body. tsc clean, auth 11/11.
- User remediation: revoke Spenza at myaccount.google.com → Security → Third-party access; Settings → Sign out → Sign in (tick all consents); log should then show "refresh token stored"; verify no mintToken 404 after >1h idle.

## 2026-07-19 — Universal per-entry "Syncing" chip (pendingSyncIds for all entry paths)
- Previously only widget-flushed entries and account adjustments were tagged into `pendingSyncIds`; entries typed in-app showed no per-entry sync state (only global banner) — user couldn't tell which logs hadn't reached Drive during the 2-day auth outage.
- Tagged now: `addEntry`, `addEntries`, `updateEntry` (re-tags an edited entry), `recordDebtPayment`'s generated Debt Payment expense. Daily chip UI unchanged (already generic on pendingSyncIds).
- Race fix in `persistToDrive`: success previously cleared `pendingSyncIds: []` wholesale — an entry added DURING the Drive write (after buildBackupDocument) lost its chip despite not being uploaded. Now captures `idsBeingPersisted` at doc-build time and clears only those; the queued follow-up persist clears the rest.
- Known limitation: entries restored from a dirty startup snapshot aren't individually tagged (dirty flag is doc-level) — global syncing status covers that window.
- tsc clean; expense-store + daily specs 90/90. Pending: rebuild + visual check (new entry shows amber chip, disappears after sync; goes amber on edit).
- Separate ACTIVE issue unchanged: refresh-token 404s until GOOGLE_OAUTH_CLIENT_SECRET is fixed + redeploy + revoke/re-consent (invalid_client confirmed 2026-07-17).

## 2026-07-19 — Sign-in test: empty Sync log diagnosed as unlogged offline→online fallback
- Device test of the refresh-token flow: account picker appeared but Settings Sync log had ZERO entries — no `auth.offlineExchange` success line.
- Elimination: every exchange outcome logs; preflight failure logs; interactive-swallowed-by-silent would show no picker. Only consistent path: `#nativeOfflineSignIn` threw before `#exchangeAuthCode` (offline `SocialLogin.login` error or missing serverAuthCode), and the `#nativeSignIn` catch fell back to online mode with only a console.warn — signed in, no refresh token, no trace.
- Fix: `recordAuthDiagnostic('auth.offlineFallback', …)` in that catch (carries the throw message, states no refresh token stored) and `recordAuthDiagnostic('auth.signInShortCircuit', …)` when an interactive sign-in is satisfied by the in-flight silent renewal (benign; distinguishes it from a swallowed popup).
- Decision: keep cancellation rethrow unlogged (user-visible abort, not a sync failure). Next: rebuild, sign in again, read auth.offlineFallback message to identify the real throw site.

## 2026-07-19 — Root cause of missing refresh token: invalid_client (wrong OAuth client secret)
- The new `auth.offlineFallback` logging paid off on the first retest: `auth.offlineExchange HTTP 400 {"googleError":"invalid_client"}` → Google's token endpoint rejects the client_id+client_secret pair. The stored GOOGLE_OAUTH_CLIENT_SECRET does not belong to web client 663004583066-vu5c… (wrong/rotated/other-client secret).
- Key distinction recorded: `invalid_client` = credentials wrong (fix the secret); `invalid_grant` = bad/expired/re-used serverAuthCode. The preflight (empty-body 400 probe) cannot detect a wrong secret VALUE — by design it only detects deployment + secret existence.
- Remedy: GCP console → spenza-notifications → Credentials → Web client secret (Add secret/rotate if not displayed) → `firebase functions:secrets:set GOOGLE_OAUTH_CLIENT_SECRET` → redeploy exchangeGoogleAuthCode + getGoogleAccessToken (secrets bind at deploy) → sign out/in.

## 2026-07-19 — Refresh-token flow RESOLVED
- After re-setting GOOGLE_OAUTH_CLIENT_SECRET with the correct Web-client secret, redeploying both token functions, and revoke-and-re-consent, the device sync log shows `auth.offlineExchange — refresh token stored`. Silent renewal enabled; hourly re-login should be gone.
- Lesson captured: the empty-log failure was invisible for two days because the offline→online fallback logged nothing; diagnostic-first debugging (add the missing log, retest, read the exact googleError) found the misconfigured secret in one iteration. Preflight design accepts it can't validate the secret VALUE — the `auth.offlineFallback` entry is now the detector for that case.
- Remaining: 1h+ soak check (no sign-in prompt, no auth.mintToken failures); disable superseded client secret in GCP console if one was added.

## 2026-07-19 — FCM token-registry hardening (owner→partner widget push)
- Symptom: partner→owner instant (FCM overlay), owner→partner only after partner opened the app (+10-20s auth-mint wait). Diagnosis: `notifyPartnerLedgerWrite` is direction-symmetric; the partner device simply had no live token in `users` (registration was Settings-toggle-only, `onNewToken` was a TODO, CF never pruned dead tokens so staleness was invisible).
- Decisions: silent startup re-registration is `tokenOnly` (never opts device into reminder scheduler, never prompts for permission — skips if not granted); family mode alone now warrants a token registration (widget two-way sync must not depend on the reminders toggle); Java→Angular rotated-token handoff via CapacitorStorage `spenza_fcm_pending_token_v1` because the Java service cannot mint a Firebase ID token for `registerToken`; CF prunes `registration-token-not-registered` always but trusts `invalid-argument` as a dead token only when the same multicast had ≥1 success (payload-error ambiguity).
- Rejected: calling registerToken CF from Java onNewToken (no auth token available); speeding up the in-app listener attach as the "fix" (it's the fallback path; push is the product behavior).

## 2026-07-24 — Circle Splits: retro-share prompt when adding a member
- User request from live trip data (Coutrallam circle): adding a member mid-trip left them out of all prior bills silently; the workaround was creating a second circle. New flow: owner adds a member → if the circle has live bills, a choice modal asks "Share all bills so far" vs "Only upcoming bills" (no bills → direct add, no prompt).
- Server: `updateCircle` CF accepts `shareExistingForNewMembers: true`. Same transaction as the add; the expenses snapshot read is hoisted BEFORE any write (Firestore txn ordering) and shared by the removal-strip and retro-include branches via one patch-map (one update per doc even when both touch it).
- RETRO RULE (deliberate): new members are appended ONLY to "everyone" splits — participant set == full pre-add member set (after any removal strip). Custom splits are never widened. Rationale: the live data itself showed payers deliberately unticking themselves (priya 4 bills, Bhuvana 1); blind append would violate author intent.
- Rejected: client-side batched writes (rules + author-permission friction, non-atomic vs concurrent member changes); date-based "share from X" option (v2 if asked); recalculating removal shares (already automatic — `expenseSharesPaise` filters to current members at compute time).
- Analysis note (same session): the "different share per member" report on the trip circle was NOT a bug — 5 of 41 bills were 11-way payer-self-excluded splits + deterministic paise rounding; shares sum exactly to the ₹45,367 total and all 12 settle-up transfers reconcile.
- Verified: functions tsc clean, app tsc clean, prod build clean, circle-settlement spec 14/14. Txn envelope: one update per everyone-split bill — same scale as the removal strip, OK below ~450 bills/circle (500-write txn limit).

## 2026-07-24 — Circle Splits family mode, delete circle, settle-relog fix
- Product context: user's live 3-family trip circle was modeled as 12 individuals; heads pay for their family, so settlement should consolidate per family. Two user decisions taken via explicit options: (a) family management = tap-based per-member "Family" select in Manage members (drag-drop and head-first nested add REJECTED — drag-drop fiddly on mobile, nested add can't retrofit live data); (b) Settle accounting = HEAD carries the whole family share into Daily, members get an acknowledgment card only (both-log alternative REJECTED — head's budget wouldn't reflect real outflow).
- Core invariant kept: families NEVER touch per-person expense shares — they aggregate resulting balances only (`familyKeyOf` = familyHeadMemberId ?? memberId; single-member groups make every function backward-compatible). All roll-ups paise-exact.
- Data model: additive optional field, no migration; live circle data untouched until owner assigns families.
- Server: `assignFamilies` partial patch in `updateCircle` validates FINAL state (referenced heads exist + point to self) → catches orphaned/nested pointers; removal of a head with members 409s. `deleteCircle` CF is the ONE deliberate exception to "tombstones, never deletes" — Admin-SDK recursiveDelete; client security rules still deny deletes.
- Settle-relog root cause: `#autoLogSettledCircle` deduped by scanning live entries for the `[circleId]` comment tag + in-memory session Set → user-deleted entry resurrected next start. Fix: persistent Preferences marker `spenza_circle_settle_logged_v1` checked before anything, written after successful log AND after deliberate skip (member covered by head). Marker is per-device by design; the comment-tag scan remains as the cross-device (Drive-sync) guard.
- Verified: settlement spec 24/24 (10 new family tests incl. no-family degeneration equality), expense-store 49/49, tsc app+functions clean, prod build clean. Full vitest has 4 PRE-EXISTING unrelated failures (subscription.service.ts:74 localStorage guard violation, ai-insight, local-notification e2e, accessibility) — backlog, not from this change.

## 2026-07-24 — Circle detail polish: rename member, sticky header, segmented tabs, ThemedSelect
- Rename is server-side in `updateCircle` (`renameMember`) because the members map is server-authoritative; display-name-only by design — memberId keys splits/families so nothing else changes. 'Name required' → 400.
- Sticky offsets: app-shell mobile bar = 36px icon-btn + 24px py-3 + 1px border ≈ 61px; desktop h-16+border ≈ 65px. Circle header card uses top-[61px]/min-[887px]:top-[65px], z-30 (below shell z-40). If shell bar height ever changes, these offsets must follow.
