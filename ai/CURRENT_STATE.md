...
- ACTIVE: silent token renewal broken for user's account — `getGoogleAccessToken` 404s (no refresh token on server). Remediation: revoke Spenza at myaccount.google.com, sign out/in in app (offline flow must run + first-consent), confirm "refresh token stored" in Settings sync log. Extra auth.offlinePreflight/offlineExchange diagnostics added 2026-07-17.
