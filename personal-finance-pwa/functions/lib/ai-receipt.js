...
exports.extractReceipt = (0, https_1.onRequest)({ cors: true, secrets: ['GROQ_API_KEY', 'GEMINI_API_KEY'] }, async (req, res) => {
    if (req.method !== 'POST') {
...
async function callGeminiWithFallbacks(apiKey, models, payload, categories) {
    ...
    for (const model of models) {
        ...
        if (response.ok) {
            ...
            if (extraction)
                return { ok: true, model, extraction };
...
function normalizeExtraction(extraction, categories) {
    ...
    const lineItems = (Array.isArray(extraction.lineItems) ? extraction.lineItems : [])
        ...
        .filter((item) => Boolean(item.name) && item.amount !== null)
        .slice(0, 30);
...
function uniqueAmounts(values) {
    ...
    for (const value of values) {
        ...
        if (amount === null || amounts.some((e) => Math.abs(e - amount) < 0.01))
            continue;
        amounts.push(amount);
        if (amounts.length >= 5)
            break;
...
//# sourceMappingURL=ai-receipt.js.map