 ...
 */
exports.sendDueReminders = (0, scheduler_1.onSchedule)('every 1 minutes', async () => {
    ...
    const resolveWebTokens = async (uid) => {
    ...
    for (const doc of dueSnap.docs) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            const d = fresh.data();
            if (!fresh.exists || !d || d.notifiedAt != null || d.status !== 'active')
                return;
            tx.update(doc.ref, { notifiedAt: firestore_1.FieldValue.serverTimestamp() });
            claimed = true;
        });
        if (!claimed)
            continue;
        const title = reminder.title?.trim() || 'You have a reminder';
    ...
    console.log(`sendDueReminders complete: ${dueSnap.size} due — ` +
        `${pushed} pushes sent, ${noToken} with no web token, ${failures} failures, ${removed} stale tokens removed`);
});
//# sourceMappingURL=send-due-reminders.js.map