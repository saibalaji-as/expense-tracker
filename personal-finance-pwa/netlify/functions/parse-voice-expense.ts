...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    const payload = JSON.parse(event.body || '{}') as AiVoiceExpensePayload;
    const transcript = cleanTranscript(payload.transcript);
    if (!transcript) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Voice transcript is required' }) };
    }
    if (transcript.length > MAX_TRANSCRIPT_LENGTH) {
      return { statusCode: 413, headers, body: JSON.stringify({ error: 'Voice transcript is too long' }) };
    }

    const categories = normalizeCategories(payload.categories);
    const generated = await callGeminiWithFallbacks(apiKey.trim(), modelCandidates(process.env.GEMINI_MODEL), {
      ...payload,
      transcript,
    }, categories);
    ...
    console.error('[parse-voice-expense] Error', {
...
> {
  ...
  for (const model of models) {
      ...
      `https://generativelanguage.googleapis.com/${GEMINI_API_VERSION}/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        ...
        body: JSON.stringify({
          contents: [
            {
              role: 'user',
              parts: [{ text: buildPrompt(payload, categories) }],
            },
          ],
          generationConfig: {
            temperature: 0.1,
    ...
    if (response.ok) {
      ...
      console.warn('[parse-voice-expense] Gemini returned unparsable expense', lastMalformed);
      continue;
    }

    lastFailure = await readGeminiFailure(response, model);
    console.error('[parse-voice-expense] Gemini request failed', lastFailure);
  ...
  return {
    ...
    clientMessage: failure.status === 401 || failure.status === 403
      ...
      : failure.status === 429
        ...
        : 'AI voice expense parsing temporarily unavailable',
...
function parseExpense(text: string): ExtractedVoiceExpense | null {
  ...
  for (const attempt of attempts) {
    try {
      return JSON.parse(attempt) as ExtractedVoiceExpense;
    } catch {
...
function normalizeExpense(expense: ExtractedVoiceExpense, categories: string[], transcript: string) {
  ...
  return {
    rawText: cleanText(expense.rawText) || transcript,
    amount,
    date,
    type,
    comment,
...
function numberOrNull(value: unknown, allowZero = false): number | null {
  ...
  return number > 0 ? Number(number.toFixed(2)) : null;
}
